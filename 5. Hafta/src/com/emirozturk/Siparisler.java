...
public class Siparisler {
    ...
    public List<Siparis> adaGoreAra(String arananAd){
        ArrayList<Siparis> sonucListesi = new ArrayList<>();
        for(var siparis:siparisListesi){
            var adSoyad = siparis.getMusteriAdSoyadi().toLowerCase(Locale.forLanguageTag("tr"));
            var aranan = arananAd.toLowerCase(Locale.forLanguageTag("tr"));
            if(adSoyad.contains(aranan)) //siparişin müşteri ad soyad alanını alınıp parametre verilen tarih ile karşılaştırılması
                sonucListesi.add(siparis); //Eğer içeriyorsa sonuç listesine eklenir.
        }
        return sonucListesi;
    }
    public List<Siparis> tumunuAl(){
